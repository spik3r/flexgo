...
// The inner Row has Flex: 1 to expand and fill available width, so Justify
// can distribute the remaining space between the fixed-width children.
	...
	return &flexgo.Node{
		...
		Children: []*flexgo.Node{
			{Height: 3, View: sectionLabel(title)},
			{
				...
				Children: []*flexgo.Node{
					{Width: 20, Background: lipgloss.Color("61"), View: cell("A")},
					{Width: 20, Background: lipgloss.Color("61"), View: cell("B")},
					{Width: 20, Background: lipgloss.Color("61"), View: cell("C")},
				},
			},
		},
	}
