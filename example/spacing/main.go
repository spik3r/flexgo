...
func initialModel() model {
	...
	// ── Padding section ───────────────────────────────────────────────────────
	// Padding shrinks the content area inside a container. Debug:true draws a
	// border around the container so the inset is clearly visible.
	paddingSection := &flexgo.Node{
		Dir:  flexgo.Col,
		Flex: 1,
		Children: []*flexgo.Node{
			{Height: 3, View: sectionHeader("Padding: 2  — inner space (debug border shows container edge)")},
			{
				Dir:     flexgo.Col,
				Flex:    1,
				Padding: 2,
				Debug:   true, // rounded border shows where the container edge is
				Children: []*flexgo.Node{
					{
						Dir:  flexgo.Row,
						Flex: 1,
	...
	marginSection := &flexgo.Node{
		...
		Children: []*flexgo.Node{
			{Height: 3, View: sectionHeader("Margin: 1  — outer space around each node")},
			{
				...
				Flex: 1,
				Children: []*flexgo.Node{
					{Flex: 1, Margin: 1, View: cell("A")},
					{Flex: 1, Margin: 1, View: cell("B")},
					{Flex: 1, Margin: 1, View: cell("C")},
				},
			},
		},
	}
